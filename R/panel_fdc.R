...
#' @title panel medQJ
#' @export
panel_medQJ = function (data_code,
    ...
    breaks_Date = function(X, date_break) {
        Xmin = min(X)
        Xmax = max(X)
    ...
    Xmin = min(data_code_obs$Date)
    Xmax = max(data_code_obs$Date)
    print(seq.Date(from=as.Date(paste0(
                 round(lubridate::year(Xmin), -1),
                 "-01-01")),
             to=as.Date(paste0(
                 round(lubridate::year(Xmax), -1),
    ...
    minor_breaks_Date = function(X, date_minor_break) {
        Xmin = min(X)
        Xmax = max(X)
        seq.Date(from=as.Date(paste0(
                     round(lubridate::year(Xmin), -1),
    ...
    if (isDate) {
        p = p +
            scale_x_date(
                ...
                date_minor_break=date_minor_break)
    } else {
        p = p +
            scale_x_continuous(
                breaks=breaks,
